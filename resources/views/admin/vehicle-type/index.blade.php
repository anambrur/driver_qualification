...
@section('content')
    <div class="p-4 mx-auto max-w-7xl md:p-6">
        ...
        <div class="mb-6">
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-white/90">Vehicle Types</h2>
                    <p class="text-gray-600 dark:text-gray-400 mt-2">Manage all vehicle types in your system</p>
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    <a href="{{ route('admin.vehicle.type.create') }}"
                        ...
                        <i class="fas fa-plus mr-2"></i>Add New Vehicle Type
                    </a>
                </div>
            </div>
        </div>

        <!-- Vehicle Types Table Card -->
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5">
                <div class="flex flex-col justify-between sm:flex-row sm:items-center">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">
                        <i class="fas fa-list mr-2"></i>Vehicle Types List
                    </h3>
                    <div class="mt-2 text-sm text-gray-600 dark:text-gray-400 sm:mt-0">
                        Total: <span id="total-records" class="font-medium">Loading...</span> vehicle types
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 dark:border-gray-800">
                <div class="p-5 sm:p-6">
                    <div class="overflow-hidden">
                        <table id="vehicle-types-table" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead>
                                <tr>
    ...
    <!-- Create/Edit Modal -->
    <div id="vehicleTypeModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
                        ...
                        <div class="w-full mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-4">
                                <form id="vehicleTypeForm">
                                    @csrf
                                    <input type="hidden" id="vehicle_type_id" name="id">
                                    <div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#vehicle-types-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route("admin.vehicle.type.index") }}',
                    type: 'GET',
                    data: function(d) {
            ...
            $('#closeModal').on('click', function() {
                $('#vehicleTypeModal').addClass('hidden');
                resetForm();
            });
            ...
            $('a[href="{{ route("admin.vehicle.type.create") }}"]').on('click', function(e) {
                e.preventDefault();
                showCreateModal();
            });

            // Edit vehicle type - using event delegation for dynamically loaded content
            $(document).on('click', 'a[href*="vehicle-type/edit"]', function(e) {
                e.preventDefault();
        ...
        function showCreateModal() {
            $('#modalTitle').text('Add New Vehicle Type');
            $('#submitText').text('Save');
            $('#vehicle_type_id').val('');
            resetFormErrors();
            $('#vehicleTypeModal').removeClass('hidden');
            $('#name').focus();
        }
        ...
        function showEditModal(id) {
            ...
            $.ajax({
                url: '{{ route("admin.vehicle.type.edit", ":id") }}'.replace(':id', id),
                type: 'GET',
                success: function(response) {
                    $('#modalTitle').text('Edit Vehicle Type');
                    $('#submitText').text('Update');
                    $('#vehicle_type_id').val(id);
                    $('#name').val(response.vehicleType.name);
                    $('#submitText').text('Update');
                },
                error: function() {
                    showToast('Failed to load vehicle type data', 'error');
                    $('#vehicleTypeModal').addClass('hidden');
                }
            });
        }
        ...
        $('#submitForm').on('click', function() {
            var formData = $('#vehicleTypeForm').serialize();
            var url = $('#vehicle_type_id').val() 
                ? '{{ route("admin.vehicle.type.update", ":id") }}'.replace(':id', $('#vehicle_type_id').val())
                : '{{ route("admin.vehicle.type.store") }}';
            var method = $('#vehicle_type_id').val() ? 'PUT' : 'POST';
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        $('#vehicleTypeModal').addClass('hidden');
                        $('#vehicle-types-table').DataTable().ajax.reload();
        ...
        function deleteVehicleType(id, name) {
            Swal.fire({
                title: 'Are you sure?',
                text: "You won't be able to revert this! Vehicle type: " + name,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route("admin.vehicle.type.destroy", ":id") }}'.replace(':id', id),
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },

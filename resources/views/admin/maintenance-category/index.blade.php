...
@section('content')
    <div class="p-4 mx-auto md:p-6">
        ...
        <div class="mb-6">
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-white/90">Maintenance Categories</h2>
                    <p class="text-gray-600 dark:text-gray-400 mt-2">Manage all maintenance categories in your system</p>
                </div>
        ...
        <!-- Maintenance Categories Table Card -->
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5">
                <div class="flex flex-col justify-between sm:flex-row sm:items-center">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">
                        <i class="fas fa-tools mr-2"></i>Maintenance Categories List
                    </h3>
                    <div class="mt-2 text-sm text-gray-600 dark:text-gray-400 sm:mt-0">
                        Total: <span id="total-records" class="font-medium">Loading...</span> categories
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 dark:border-gray-800">
                <div class="p-5 sm:p-6">
                    <div class="overflow-hidden">
                        <table id="maintenance-categories-table"
    ...
    <!-- Create/Edit Modal -->
    <div id="maintenanceCategoryModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
                ...
                <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        <div class="w-full mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-4">
                                <form id="maintenanceCategoryForm">
                                    @csrf
                                    <input type="hidden" id="maintenance_category_id" name="id">
                                    <div>
                                        <label for="name"
                                        ...
                                        <input type="text" name="name" id="name" required
                                            ...
                                            placeholder="Enter category name (e.g., Oil Change, Engine Repair, Brake Service)">
                                        <div id="name_error" class="mt-1 text-sm text-red-600"></div>
                                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                            Enter a descriptive name for the maintenance category
                                        </p>
                                    </div>
                                </form>
                            </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            var table = $('#maintenance-categories-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: '{{ route('admin.maintenance.category.index') }}',
                    type: 'GET',
                    error: function(xhr, error, thrown) {
                        console.error('DataTables AJAX Error:', error);
                        showToast('Failed to load maintenance categories', 'error');
                    }
                },
                columns: [{
                        data: 'DT_RowIndex',
                ...
                language: {
                    ...
                    emptyTable: "No maintenance categories found",
            ...
            $('#closeModal').on('click', function() {
                $('#maintenanceCategoryModal').addClass('hidden');
                resetForm();
            });
            ...
            $('#submitForm').on('click', function() {
                submitMaintenanceCategoryForm();
            });
        });

        // Show create modal
        function showCreateModal() {
            $('#modalTitle').text('Add New Maintenance Category');
            $('#submitText').text('Save');
            $('#maintenance_category_id').val('');
            resetFormErrors();
            $('#maintenanceCategoryModal').removeClass('hidden');
            $('#name').focus();
        }

        // Show edit modal
        function editMaintenanceCategory(id) {
            resetFormErrors();
            ...
            $.ajax({
                url: '{{ route('admin.maintenance.category.edit', ':id') }}'.replace(':id', id),
                type: 'GET',
                success: function(response) {
                    if (response.success) {
                        $('#modalTitle').text('Edit Maintenance Category');
                        $('#submitText').text('Update');
                        $('#maintenance_category_id').val(id);
                        $('#name').val(response.data.name);
                    } else {
                        showToast('Failed to load maintenance category data', 'error');
                        $('#maintenanceCategoryModal').addClass('hidden');
                    }
                },
                error: function() {
        ...
        // Submit form
        function submitMaintenanceCategoryForm() {
            var formData = $('#maintenanceCategoryForm').serialize();
            var url = $('#maintenance_category_id').val() ?
                '{{ route('admin.maintenance.category.update', ':id') }}'.replace(':id', $('#maintenance_category_id')
                .val()) :
                '{{ route('admin.maintenance.category.store') }}';
            var method = $('#maintenance_category_id').val() ? 'PUT' : 'POST';
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        $('#maintenanceCategoryModal').addClass('hidden');
                        $('#maintenance-categories-table').DataTable().ajax.reload();
        ...
        function deleteMaintenanceCategory(id, name) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route('admin.maintenance.category.destroy', ':id') }}'.replace(':id', id),
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
                        ...
                        error: function(xhr) {
                            Swal.close();
                            let errorMessage = 'Failed to delete maintenance category.';
        ...
        // Reset form
        function resetForm() {
            $('#maintenanceCategoryForm')[0].reset();
            $('#maintenance_category_id').val('');
            resetFormErrors();
        }
        ...
        window.editMaintenanceCategory = editMaintenanceCategory;
        window.deleteMaintenanceCategory = deleteMaintenanceCategory;
